/**
 * Master Z1 teleop arm — C++ / unitree_arm_sdk
 *
...
// 主端 SDK 连接 z1_controller 的 IP（本机 controller 一般为 127.0.0.1）
#define MASTER_ARM_IP "127.0.0.1"

// UDP 端口（与 z1_controller ARMSDK 8072/8071 配对；与从端 8073/8074 不同）
...
static CtrlComponents* createCtrlComponents() {
  ...
  ctrl->dt = 0.002;
  ctrl->udp =
      new UDPPort(MASTER_ARM_IP, MASTER_LOCAL_PORT, MASTER_REMOTE_PORT, RECVSTATE_LENGTH, BlockYN::NO, 500000);
...
class MasterZ1Session {
 public:
  MasterZ1Session() {
    ctrl_ = createCtrlComponents();
    arm_ = new unitreeArm(ctrl_);
    arm_->sendRecvThread->start();
    ROS_INFO("master: Z1 SDK UDP %s  local=%u  remote=%u", MASTER_ARM_IP, MASTER_LOCAL_PORT, MASTER_REMOTE_PORT);
  }
  ...
  void moveToInitJointPose() {
    ...
    ROS_INFO("master: control mode JOINTCTRL for init pose only [0, 1.5, -1, -0.54, 0, 0]");
  ...
  void moveToVerticalHome() {
    ROS_INFO("master: backToStart (vertical home)");
    arm_->backToStart();
  }

  void startCartesianTrack() {
    // Debug logging change: make the teleop control mode explicit.
    ROS_INFO("master: control mode CARTESIAN active; command=cartesianCtrlCmd");
...
class MasterTeleopArm {
 public:
  MasterTeleopArm() {
    ...
    desired_pub_ = nh_.advertise<geometry_msgs::Pose>("/master/desired_pose", 10);
    pose_pub_ = nh_.advertise<geometry_msgs::PoseStamped>("/master_arm/tool_pose", 10);
    vel_pub_ = nh_.advertise<geometry_msgs::TwistStamped>("/master_arm/tool_velocity", 10);
    wrench_pub_ = nh_.advertise<geometry_msgs::WrenchStamped>("/master_arm/wrench", 10);
    teleop_sub_ = nh_.subscribe("/master/teleop_frame", 10, &MasterTeleopArm::teleopCb, this);

    std::string force_topic = "/master_force";
    pnh.param("force_sensor_topic", force_topic, force_topic);
    force_sub_ = nh_.subscribe(force_topic, 10, &MasterTeleopArm::forceCb, this);
    ROS_INFO("master: force sensor topic %s", force_topic.c_str());

    hw_ = std::make_unique<MasterZ1Session>();
    hw_->moveToInitJointPose();
    init_pose_ = hw_->currentPose6();
    current_pose_ = init_pose_;
    hw_->startCartesianTrack();
    state_thread_ = std::thread(&MasterTeleopArm::stateLoop, this);

    phase_ = TeleopPhase::INIT_WAIT;
    init_wait_start_ = ros::Time::now();
    rate_hz_ = static_cast<int>(1.0 / dt_);

    ROS_INFO("master: at init joint pose; CARTESIAN hold for %.0fs then admittance %.0fs", init_wait_sec_,
             run_duration_sec_);
  }
  ...
  void finishAndExit() {
    ...
    ROS_INFO("master: session end — backToStart and PASSIVE; CARTESIAN teleop stopped");
    if (hw_) {
    ...
    while (ros::ok() && !shutdown_) {
      ...
      if (phase_ == TeleopPhase::INIT_WAIT) {
        ...
        if (wait_left != last_wait_log && wait_left >= 0) {
          ROS_INFO_THROTTLE(5.0, "master: init hold, admittance in %ds", wait_left);
        ...
        if (elapsed >= init_wait_sec_) {
          ...
          ROS_INFO("master: CARTESIAN admittance teleop started (%.0fs)", run_duration_sec_);
        } else {
 ...
 private:
  ...
  void finalizeForceZero() {
    ...
    if (force_zero_samples_ == 0) {
      ROS_WARN("master: no force sensor samples during init; force zero offset remains 0");
      force_zeroed_ = true;
      return;
    }
    for (int i = 0; i < 6; ++i) {
    ...
    force_zeroed_ = true;
    ROS_INFO("master: force zero offset from %zu samples: [%.3f, %.3f, %.3f, %.3f, %.3f, %.3f]",
  ...
  void logDebugState(const Pose6& pose, const geometry_msgs::TwistStamped& vel, const Wrench6& wrench) const {
    ...
    ROS_INFO_THROTTLE(period,
                      "master debug: real_pose(base_link) [x y z r p y]=[%.3f %.3f %.3f %.3f %.3f %.3f]",
                      pose[0], pose[1], pose[2], pose[3], pose[4], pose[5]);
    ROS_INFO_THROTTLE(period,
                      "master debug: cal_vel(base_link) linear=[%.3f %.3f %.3f] angular_rpy=[%.3f %.3f %.3f]",
                      vel.twist.linear.x, vel.twist.linear.y, vel.twist.linear.z, vel.twist.angular.x,
                      vel.twist.angular.y, vel.twist.angular.z);
    ROS_INFO_THROTTLE(period,
                      "master debug: real_wrench(base_link zeroed/world) force=[%.3f %.3f %.3f] torque=[%.3f %.3f %.3f] force_feedback=%s",
  ...
  Wrench6 admittanceForce() const {
    ...
    if (!enable_force_feedback_ || !has_teleop_frame_) {
      ...
      if (period > 0.0) {
        ...
        ROS_INFO_THROTTLE(period,
                          "master debug: admittance_force input=[%.3f %.3f %.3f %.3f %.3f %.3f] force_feedback=%s remote_frame=%s",
    ...
    if (period > 0.0) {
      ...
      ROS_INFO_THROTTLE(period,
                        "master debug: admittance_force local=[%.3f %.3f %.3f %.3f %.3f %.3f] remote=[%.3f %.3f %.3f %.3f %.3f %.3f] input=[%.3f %.3f %.3f %.3f %.3f %.3f]",
  ...
  std::unique_ptr<MasterZ1Session> hw_;
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "master_arm");
  try {
    MasterTeleopArm node;
    node.run();
  } catch (const std::exception& e) {
    ROS_FATAL("master_arm: %s", e.what());
    return 1;
  }
  return 0;
}
