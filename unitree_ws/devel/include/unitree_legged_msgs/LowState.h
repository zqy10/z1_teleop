...
struct MD5Sum< ::unitree_legged_msgs::LowState_<ContainerAllocator> >
{
  ...
  {
    return "481f8a83f729a25e7cc170da192f80f9";
  }

  static const char* value(const ::unitree_legged_msgs::LowState_<ContainerAllocator>&) { return value(); }
  static const uint64_t static_value1 = 0x481f8a83f729a25eULL;
  static const uint64_t static_value2 = 0x7cc170da192f80f9ULL;
};
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::unitree_legged_msgs::LowState_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.eeForceRaw.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::unitree_legged_msgs::Cartesian_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.eeForceRaw[i]);
    }
    if (v.eeForceRaw.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.eeForce.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::unitree_legged_msgs::Cartesian_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.eeForce[i]);
    }
    if (v.eeForce.empty() || false)
      s << "]";
    if (true || !indent.empty())
