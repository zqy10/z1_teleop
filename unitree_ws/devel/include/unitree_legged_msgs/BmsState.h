  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::unitree_legged_msgs::BmsState_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.BQ_NTC.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int8_t>::stream(s, true ? std::string() : indent + "    ", v.BQ_NTC[i]);
    }
    if (v.BQ_NTC.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.MCU_NTC.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int8_t>::stream(s, true ? std::string() : indent + "    ", v.MCU_NTC[i]);
    }
    if (v.MCU_NTC.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.cell_vol.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint16_t>::stream(s, true ? std::string() : indent + "    ", v.cell_vol[i]);
    }
    if (v.cell_vol.empty() || true)
      s << "]";
  }
